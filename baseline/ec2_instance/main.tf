# Debian 11 Bullseye
data "aws_ami" "debian_11" {
  most_recent = true
  owners      = ["136693071363"]
  filter {
    name   = "name"
    values = ["debian-11-amd64-*"]
  }
  filter {
    name   = "virtualization-type"
    values = ["hvm"]
  }
}

resource "aws_key_pair" "groupe10_key" {
  key_name   = "tf-${var.instance_name}-key"
  public_key = "ssh-rsa "
}

resource "aws_iam_instance_profile" "groupe10_profile" {
  name = "tf-${var.instance_name}-instance-profile"
  role = aws_iam_role.groupe10_role.name
}

resource "aws_instance" "groupe10_ec2" {
  ami                  = data.aws_ami.debian_11.id
  instance_type        = "t3.micro"
  security_groups      = [aws_security_group.groupe10_sg.name]
  key_name             = aws_key_pair.groupe10_key.id
  iam_instance_profile = aws_iam_instance_profile.groupe10_profile.name
  user_data            = file("${path.module}/scripts/install_lab.sh")
  root_block_device {
    delete_on_termination = true
  }
  tags = {
    Name = "tf-${var.instance_name}-ec2"
  }
}

resource "aws_eip" "groupe10_eip" {
  vpc = true
  tags = {
    Name = "tf-${var.instance_name}-eip"
  }
}

resource "aws_default_vpc" "default" {
  tags = {
    Name = "Default VPC"
  }
}

resource "aws_eip_association" "eip_assoc" {
  instance_id   = aws_instance.groupe10_ec2.id
  allocation_id = aws_eip.groupe10_eip.id
}

resource "aws_security_group" "groupe10_sg" {
  name        = "tf-${var.instance_name}-sg"
  description = "Allow SSH & HTTPS traffic from current ip"
  vpc_id      = aws_default_vpc.default.id

  ingress {
    description = "Allow all ingress"
    from_port   = 0
    to_port     = 65535
    protocol    = "tcp"
    cidr_blocks = ["${var.my_ip}/32"]
  }

  egress {
    description = "Allow all egress"
    from_port   = 0
    to_port     = 65535
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }

  tags = {
    Name = "tf-${var.instance_name}-sg"
  }
}
